...
<%--    </c:if> --%>
   ...
   <!-- 이전 페이지로 가기 -->
   <c:if test="${paging.curPage ne 1 }">
   <li><a href="/qna/list?curPage=${paging.curPage - 1 }" class="paging_num">&lt;</a>
   </c:if>
   ...
   <li class="on"><a href="/qna/list?curPage=${i }">${i }</a></li>
   </c:if>
   
   <!-- 현재 페이지가 아니라면 평소 모습-->
   <c:if test="${paging.curPage ne i }">
   <li><a href="/qna/list?curPage=${i }"class="paging_num">${i }</a></li>
   </c:if>
...
<%--    </c:if> --%>
   ...
   <!-- 마지막 페이지로 가기 -->
   <c:if test="${paging.curPage ne paging.totalPage }">
   <li><a href="/qna/list?curPage=${paging.totalPage }">&raquo;</a></li>
   </c:if>
